...
try {
    ...
    Invoke-PreDeploy -Postgres @{ Host = $env:POSTGRES_HOST ; DbName = $env:POSTGRES_DB_NAME } `
    ...
    Write-LogInfo "Starting Deploy..."
    Invoke-Deploy -PostgreHost $env:POSTGRES_HOST -PostgrePort $env:POSTGRES_PORT `
    ...
    Invoke-PostDeploy -PostgresHost $env:POSTGRES_HOST `
                         -DbName $env:POSTGRES_DB_NAME -DbUserName $env:POSTGRES_DB_USERNAME `
